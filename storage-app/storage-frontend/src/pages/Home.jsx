      ...
      <div className="max-w-5xl mx-auto p-4 md:p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
          <div className="p-4 bg-white rounded-lg shadow-sm">
            ...
            <div className="flex">
              <input
                className="flex-1 border px-3 py-2 rounded-l-md focus:ring-emerald-500 focus:border-emerald-500"
                value={folderName}
                onChange={(e) => setFolderName(e.target.value)}
                placeholder="Folder name"
              />
              <button
                className="px-4 py-2 bg-emerald-600 text-white rounded-r-md hover:bg-emerald-700 font-semibold"
          ...
          {!loading && items.length > 0 && (
            <ul>
              {items.map((it) => (
                <li
                  key={it.name}
                  className="flex items-center justify-between py-3 border-b border-gray-100 hover:bg-slate-50"
                >
                  ...
                  <div className="flex items-center space-x-2">
                    {it.isFolder ? (
                      <button
                        className="px-3 py-1 text-sm bg-gray-100 border border-gray-300 text-gray-700 rounded-md hover:bg-gray-200"
                        onClick={() => openFolder(it.name)}
                      >
                        Open
                      </button>
                    ) : (
